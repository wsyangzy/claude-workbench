...
export const CCAgents: React.FC<CCAgentsProps> = ({ onBack, className }) => {
  ...
  const loadAgents = async () => {
    try {
      ...
      setError(t('common.failedToLoadAgents'));
      setToast({ message: t('common.failedToLoadAgents'), type: "error" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const confirmDeleteAgent = async () => {
    ...
    try {
      ...
      setToast({ message: t('common.agentDeletedSuccessfully'), type: "success" });
      await loadAgents();
      await loadRuns(); // Reload runs as they might be affected
    } catch (err) {
      console.error("Failed to delete agent:", err);
      setToast({ message: t('common.failedToDeleteAgent'), type: "error" });
    } finally {
      setIsDeleting(false);
  ...
  const handleAgentCreated = async () => {
    setView("list");
    await loadAgents();
    setToast({ message: t('common.agentCreatedSuccessfully'), type: "success" });
  };

  const handleAgentUpdated = async () => {
    setView("list");
    await loadAgents();
    setToast({ message: t('common.agentUpdatedSuccessfully'), type: "success" });
  };
  ...
  const handleExportAgent = async (agent: Agent) => {
    try {
      ...
      setToast({ message: t('common.agentExportSuccess', { name: agent.name }), type: "success" });
    } catch (err) {
      console.error("Failed to export agent:", err);
      setToast({ message: t('common.agentExportFailed'), type: "error" });
    }
  };
  ...
  const handleImportAgent = async () => {
    try {
      ...
      setToast({ message: t('common.agentImportSuccess'), type: "success" });
      await loadAgents();
    } catch (err) {
      console.error("Failed to import agent:", err);
      const errorMessage = err instanceof Error ? err.message : t('common.agentImportFailed');
    ...
    <div className={cn("flex flex-col h-full bg-background", className)}>
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold">{t('common.ccAgentsTitle')}</h1>
                <p className="text-sm text-muted-foreground">
                  {t('common.manageCCAgents')}
                </p>
              </div>
            </div>
              ...
              <div>
                {loading ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-64 text-center">
                    ...
                    <h3 className="text-lg font-medium mb-2">{t('common.noAgentsYet')}</h3>
                    <p className="text-sm text-muted-foreground mb-4">
                      {t('common.createFirstAgent')}
                    </p>
                            ...
                            <Card className="h-full hover:shadow-lg transition-shadow">
                              <CardContent className="p-6 flex flex-col items-center text-center">
                                ...
                                <p className="text-xs text-muted-foreground">
                                  {t('common.createdLabel')}{new Date(agent.created_at).toLocaleDateString()}
                                </p>
                              </CardContent>
                              <CardFooter className="p-4 pt-0 flex justify-center gap-1 flex-wrap">
                                <Button
                                  ...
                                  title={t('common.executeAgentTitle')}
                                >
                                  <Play className="h-3 w-3" />
                                  {t('common.executeAgent')}
                                </Button>
                                <Button
                                  size="sm"
                                  variant="ghost"
                                ...
                                <Button
                                  ...
                                  className="flex items-center gap-1"
                                  title={t('common.exportAgentTitle')}
                                >
              ...
              {!loading && agents.length > 0 && (
                <div className="overflow-hidden">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h2 className="text-lg font-semibold">{t('common.recentExecutions')}</h2>
                  </div>
                  {runsLoading ? (
      ...
      <GitHubAgentBrowser
        ...
        onImportSuccess={async () => {
          ...
          await loadAgents();
          setToast({ message: t('common.importFromGitHubSuccess'), type: "success" });
        }}
      />
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              {t('common.deleteAgentDialog')}
            </DialogTitle>
            <DialogDescription>
              {t('common.deleteAgentConfirm', { name: agentToDelete?.name })}
            </DialogDescription>
          </DialogHeader>
