...
export const MarkdownEditor: React.FC<MarkdownEditorProps> = ({
  ...
  const loadSystemPrompt = async () => {
    try {
      ...
      setError(t('files.markdownEditor.failedToLoad'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    try {
      ...
      setToast({ message: t('files.markdownEditor.savedSuccessfully'), type: "success" });
    } catch (err) {
      console.error("Failed to save system prompt:", err);
      setError(t('files.markdownEditor.failedToSave'));
      setToast({ message: t('files.markdownEditor.failedToSave'), type: "error" });
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleBack = () => {
    if (hasChanges) {
      const confirmLeave = window.confirm(
        t('files.markdownEditor.unsavedChanges')
      );
      ...
      <div className="w-full max-w-5xl mx-auto flex flex-col h-full">
        ...
        >
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-lg font-semibold"> {t('files.markdownEditor.title')} </h2>
              <p className="text-xs text-muted-foreground">
                {t('files.markdownEditor.description')}
              </p>
            </div>
          </div>
          ...
          >
            ...
            {saving ? t('files.markdownEditor.saving') : t('files.markdownEditor.save')}
          </Button>
        </motion.div>
